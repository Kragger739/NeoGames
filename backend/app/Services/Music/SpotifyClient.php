...
class SpotifyClient
{
    ...
    /**
     * Every album + single for an artist, de-duplicated by lowercased name so
     * a "Greatest Hits", a deluxe re-issue and per-territory pressings don't
     * each contribute their whole tracklist. Paginated on `next`.
     *
     * @return array<int, array{id: string, name: string}>
     */
    public function artistAlbums(string $artistId): array
    {
        $out = [];
        $seen = [];
        $offset = 0;
        ...
        do {
            $body = $this->get("/artists/{$artistId}/albums", array_filter([
            ...
            foreach ($body['items'] ?? [] as $album) {
                if (! isset($album['id'], $album['name'])) {
                    continue;
                }

                $key = mb_strtolower(trim((string) $album['name']));
                ...
                $seen[$key] = true;
                $out[] = ['id' => (string) $album['id'], 'name' => (string) $album['name']];
            }
    ...
    /**
     * The simplified track objects on an album (id, name, artists - NO
     * popularity). Paginated on `next`. `artist_ids` lets the caller drop
     * "appears on" / various-artists filler that isn't the act's own
     * recording.
     *
     * @return array<int, array{id: string, name: string, artist_ids: array<int, string>}>
     */
    public function albumTracks(string $albumId): array
    {
        $out = [];
        $offset = 0;

        do {
            $body = $this->get("/albums/{$albumId}/tracks", array_filter([
                'market' => config('music.spotify_market', 'US'),
                'limit' => 50,
                'offset' => $offset,
            ], fn ($x) => $x !== null && $x !== ''));
            ...
            foreach ($body['items'] ?? [] as $track) {
                ...
                $out[] = [
                    ...
                    'artist_ids' => array_values(array_filter(array_map(
                        fn ($a) => isset($a['id']) ? (string) $a['id'] : null,
                        $track['artists'] ?? [],
                    ))),
                ];
            }
    ...
    /**
     * Full track objects for any number of ids, fetched in chunks of 50 and
     * each normalized (so `popularity` and `release_year` are present). Order
     * is not preserved; market-unavailable nulls are dropped.
     *
     * @param  array<int, string>  $ids
     * @return array<int, NormalizedSpotifyTrack>
     */
    public function tracksByIds(array $ids): array
    {
        $ids = array_values(array_unique(array_filter($ids)));
        $out = [];

        foreach (array_chunk($ids, 50) as $chunk) {
            $body = $this->get('/tracks', [
                'ids' => implode(',', $chunk),
                'market' => config('music.spotify_market', 'US'),
            ]);
